...
const RESERVED_USERNAMES = ["api", "assets", "from", "docs", "blog", "about"];
app.post(chooseUserName, async (req, res) => {
  ...
  if (!username.match(/^[\w\d_-]+$/)) {
    return res
      ...
      .json({
        error:
          "Usernames can only have letters, digits, underscores, and dashes.",
      })
      .end();
  }
...
app.post(share, async (req, res) => {
  ...
  if (!name.match(/^[\w\d_-]+$/)) {
    return res
      ...
      .json({
        error:
          "Timeline names can only contain letters, digits, underscores, and dashes.",
      })
      .end();
  }
...
async function getAllUserNamesAndIds() {
  const users = (await getAllUserIds()).map((userId) =>
    admin.firestore().doc(`users/${userId}`)
  );
  const usernames = await admin
    .firestore()
    .getAll(...users, { fieldMask: ["username"] });
  return usernames
    .filter((userDoc) => userDoc.exists && !!userDoc.data().username)
    .map((userDoc) => {
      return {
...
app.get(cascade, async (req, res) => {
  const path = `${req.params.username}/${
    req.params.cascade ? req.params.cascade : req.params.username
  }`;
...
app.get("/:user/:timeline?", async (req, res, next) => {
  ...
  const path = `${userId}/${
    req.params.timeline ? req.params.timeline : req.params.user
  }`;
  const file = admin
    .storage()
    .bucket()
  ...
  req.timelinePath = `${req.params.user}${
    req.params.timeline ? "/" + req.params.timeline : ""
  }`;
  next();
});
